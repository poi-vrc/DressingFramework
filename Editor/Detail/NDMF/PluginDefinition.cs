...
namespace Chocopoi.DressingFramework.Detail.NDMF
{
    internal class PluginDefinition : Plugin<PluginDefinition>
    {
        ...
        private static Tuple<BuildStage, BuildStage> NDMFPhaseToDKStage(BuildPhase phase)
        {
            ...
            {
                return new Tuple<BuildStage, BuildStage>(BuildStage.Pre, BuildStage.Analyze);
            }
        ...
        private void ConfigureStages(PluginManager pluginMgr, BuildPhase ndmfPhase, BuildStage startStage, BuildStage endStage)
        {
            ...
            {
                // TODO
                var hooks = pluginMgr.GetSortedBuildPassesAtStage(stage) ?? throw new Exception("DK has unresolved or cyclic hook dependencies.");
                ...
                foreach (var hook in hooks)
                {
                    ...
                    foreach (var runtimeHook in hook.Constraint.afterRuntimeHooks)
                    {
                    ...
                    seq.WithRequiredExtension(typeof(DKExtensionContext), extSeq =>
                    {
                        ...
                        foreach (var runtimeHook in hook.Constraint.beforeRuntimeHooks)
                        {
                            // NDMF has made the qualified name version of BeforePass private
                            // pass.BeforePass(runtimeHook.identifier);

                            // now we are using a hack to access this function
                            typeof(DeclaringPass).GetMethod("BeforePass").Invoke(extSeq, new object[] { runtimeHook.identifier });
                        }
                    });
