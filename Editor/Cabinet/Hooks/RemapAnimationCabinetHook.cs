...
namespace Chocopoi.DressingFramework.Cabinet.Hooks
{
    public class RemapAnimationCabinetHook : CabinetHookBase
    {
        ...
        public override bool Invoke(ApplyCabinetContext cabCtx)
        {
            foreach (var clipContainer in cabCtx.animationStore.Clips)
            {
                ...
                foreach (var curveBinding in curveBindings)
                {
                    ...
                    var newPath = cabCtx.pathRemapper.Remap(curveBinding.path, avoidContainerBones);
                ...
                {
                    var newPath = cabCtx.pathRemapper.Remap(objRefBinding.path, false);
